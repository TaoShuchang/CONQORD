...
class TwoLayerMLP(nn.Module):
    ...
    def forward(self, x):
        # 前向传播输入x
        x = self.hidden(x)  # 输入层到隐藏层
        x = self.relu(x)    # 隐藏层激活函数
        x = self.output(x)  # 隐藏层到输出层
        return x
...
class DeepSpeedRLHFEngine():
    ...
    def __init__(self, actor_model_name_or_path, critic_model_name_or_path,
        ...
        # self.critic_model1 = TwoLayerMLP(self.args.max_answer_seq_len + self.args.max_prompt_seq_len + 1, self.args.max_answer_seq_len + self.args.max_prompt_seq_len + 1, self.args.max_answer_seq_len + self.args.max_prompt_seq_len - 1).to(next(self.actor.parameters()).device)
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        # Model
        print("actor_model_name_or_path",actor_model_name_or_path)
